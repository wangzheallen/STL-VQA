...
class Answer_Generator():
    ...
    def build_model(self, is_training):
        ...
        ques_aff = tf.batch_matmul(ques_local, tf.transpose(img_local, [0, 2, 1]))
        ans_aff = tf.batch_matmul(ans_local, tf.transpose(img_local, [0, 2, 1]))
        # 500 * 26 * 49
        ...
        image_emb = tf.reduce_sum(tf.batch_matmul(att, img_local), 1)
        # 500 * 2048
    ...
    def build_generator(self, is_training):
        ...
        return generated_ANS, image, question, answer, question_length, answer_length, ques_pos, ans_pos
...
# Data input setting
input_img_h5_train = './data/spatial_data_img_residule_train_7by7.h5'
input_img_h5_test = './data/spatial_data_img_residule_test_7by7.h5'
input_ques_h5 = './data_prepro_0417_v1.h5'
input_img_h5 = './data_img_0417_v1.h5'
...
def train():
    ...
    for itr in range(max_itr):
        ...
        for idx in xrange(0, num_train/4-1, batch_size/3):
            ...
            curr_neg = [np.random.choice([id+1, id+2, id+3], 2, replace = False) for id in curr_pos]
            ...
            if flag or (itr == 0 and idx == 0):
                ...
                tf_proba_test, tf_image_test, tf_question_test, tf_answer_test, tf_question_test_length, tf_answer_test_length, tf_ques_pos_test, tf_ans_pos_test = model.build_generator(False)
                ...
                print ("Testing done.")
		'''
                if acc*1.0/len(result) > Max:
                    Max = acc*1.0/len(result)
                    saver.save(sess, 'model_7w' + string(Max))
		'''
                ...
                if acc*1.0/len(result) > Max:
		    Max = acc *1.0/len(result)
		    saver.save(sess, 'model_7w' + str(Max))
		print('Accuracy of test: ' + str(acc*1.0/len(result)))
                f2 = open("test_acc_0425.txt", "a")
